...
"%JAVA%" "%JVM_PARAMS%" -jar "%AGENT_HOME%\lib\%JAR%" "--logging.config=%AGENT_HOME%\etc\logback.xml" "--agent.home=%AGENT_HOME%" --client.id=%CLIENT_ID% --client.apiKey=%API_KEY% --agent.scale=%SCALE% --agent.colo.enabled=%COLO%
